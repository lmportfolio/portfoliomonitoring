...
with tab1:
    ...
    if not uploaded:
        ...
        try:
            ...
            st.dataframe(port_mon, use_container_width=True)
...
with tab2:
    ...
    **Objective:**  
    ...
    if not df_lob.empty:
        ...
        sns.heatmap(
            heat, annot=False, cmap=cmap, norm=norm,
            cbar_kws={"label":"Avg YTD_diff ($)",
            ...
            if sel_words:
                ...
                st.dataframe(
                    df_ctx[[
                        ...
                        Sentiment=lambda d: d["Sentiment"].map("{:+.2f}".format)
                    ),
                    use_container_width=True
                )
